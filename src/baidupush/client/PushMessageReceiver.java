 ...
 // 通知栏新消息条目，我只是用了一个全局变量，
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
            ...
            try {
                ...
                parseMessage(msgItem,context);// 预处理，过滤�?些消息，比如说新人问候或自己发�?�的
            } catch (Exception e) {
    ...
    private void parseMessage(Message msg, Context context) {
        ...
        if (!TextUtils.isEmpty(tag)) {// 如果是带有tag的消息
            ...
            PushApplication.getInstance().getUserDB().addUser(u);// 存入或更新好�?
...
//                   T.show(context, "请到百度开发者官网生成新的push key，替换", 1);
                	Log.e("fff", "用户id为空1");
                    return;
                }
    ...
    private void showNotify(Message message) {
     ...
     // 更新通知栏
        ...
        Intent intent = new Intent(application, ChatActivity.class);
     ...
     // 下面是4.0通知栏api
        ...
        application.getNotificationManager().notify(NOTIFY_ID, notification);// 通知�?下才会生效哦
    }
